...
2. The persistent HTTP MCP broker binds to `127.0.0.1:17840`. It keeps one isolated JavaScript REPL per operator session while allowing independent sessions to execute concurrently.
...
5. The native driver is a signed macOS app installed in `~/Applications`. Commands identify an exact process and window and use fresh accessibility tokens.
